...
const index = () => {
  ...
  // const [dataMerchan, setDataMerchan] = useState<any>([])//table数据
  ...
  const fetch = useCallback(
    (pagination: any = { page: 1, page_size: 10 }) => {
      setLoading(true)
        ...
        queryMerchandise(data).then(res => {
          console.log(res,898898)
          setData({
            list: res.results,
            pagination: {
              total: res.count,
        ...
        <Row className={styles.header}>
          ...
          <Col span={8} style={{ textAlign: 'center' }}>
            ...
            <Button
              type="primary"
              onClick={() => {
                history.push('/commodityManagement/listOfMerchandise/add')
  ...
  const updateMerchand = async (text: any, record: any) => {
    const hideLoading = message.loading('加载中')
    record.status =text
    updaMerchandise(record)
    .then(res => {
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <>
          <a
            onClick={() => {
              history.push(`/commodityManagement/listOfMerchandise/${record.id}`)
            }}
          >
            编辑
          </a>
          <Divider type="vertical" />
          {/* <a onClick={() => updateMerchand(true, record)}>下架</a> */}
          <TableDeleteBtn colorText={text.status=='1'?'#a9a9a9':'#1890ff'} children ={text.status=='0'?'上架':'下架'} title ={ `是否确认${text.status=='0'?'上架':'下架'}？`}
           onDelete={() => updateMerchand(text.status=='0'?'1':'0',record)} />
          <Divider type="vertical" />
