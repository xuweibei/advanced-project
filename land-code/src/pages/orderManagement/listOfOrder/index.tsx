...
import ListOfOrder from '../detailsOrder'
...
const index = (props:any) => {
  ...
  const [dataOrder, setDataOrder] = useState<any>([]) //table数据
    ...
    return (
      <Form form={form} onFinish={handleSearch} layout="inline">
        <Row className={styles.header}>
          <Col span={6}>
            <FormItem label="订单号" name="orderSn">
              <Input />
            </FormItem>
          </Col>
          <Col span={6} style={{ paddingLeft: 10 }}>
            <FormItem label="商品名" name="goodsName">
              <Input />
            </FormItem>
          </Col>
          <Col span={6} style={{ paddingLeft: 10 }}>
            <FormItem label="订单状态" name="status">
               ...
               */}
              <Select allowClear defaultValue={"选择订单状态"}>
          ...
          <Col span={6} style={{ paddingLeft: 10 }}>
            <FormItem label="支付方式" name="subTenderType">
            <Select allowClear defaultValue={"选择支付方式"}>
                {subTenderType.map((item: any, index: any) => {
                  return (
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any) =>{
      let v = Status.map((item:any)=>{
        if(item.id===text){return item.name} '--'})
      return v
      }
    },
    {
    ...
    {
      title: '支付方式',
      dataIndex: 'subTenderType',
      render: (text: any) => {
        let v = subTenderType.map((item:any)=>{
          if(item.label===text){return item.value} '--'})
        return v
        }
    },
    {
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <>
          <a
            onClick={() => {
              history.push(`/orderManagement/listOfOrder/${record.id}`)
            }}
          >
            详情
          </a>
