...
const index = () => {
  ...
  const fetcher = useQuery(
    ['/home_user/query', currPagination, currCommunityId],
    async () => {
      const values = await form.validateFields()
      const formData = {
        ...values,
        blockId: values?.building?.[0] ?? null,
        unitId: values?.building?.[1] ?? null,
        roomId: values?.building?.[2] ?? null,
      }
      ...
      const res: any = await queryHomeUsers(data)
      return {
        list: res.results,
        pagination: {
          total: res.count,
          current: currPagination.page,
          pageSize: currPagination.page_size,
        },
      }
    },
    {
      enabled: !!currCommunityId,
    }
  )
  ...
  const blockList = useQuery(
    ['buildingBlockUnitHomes', currCommunityId],
    async () => {
      return buildingBlockUnitHomes(currCommunityId).then(res => res.blockList)
    },
    {
      enabled: !!currCommunityId,
    }
  ...
  const loading = fetcher?.isFetching ?? blockList?.isFetching
  ...
  //三级联动选择楼栋-单元-户室号
  const options: any[] = blockList?.data?.map((block: any) => ({
    value: block.id,
    label: block.blockName,
    children: block?.unitList?.map((unit: any) => ({
      value: unit.id,
      label: unit.unitName,
      children: unit?.homeInfoList?.map((home: any) => ({
        value: home.roomNo,
        label: home.roomNo,
      })),
    })),
  ...
  const renderForm = () => {
    return (
      <Form form={form} onFinish={fetch} layout="inline">
            ...
            {(fields, { add, remove }) => (
              <>
                ...
                <Form.Item
                  ...
                  wrapperCol={{ span: 24 }}
                  extra={
                    <span style={{ color: 'red', display: 'block', marginTop: 16 }}>
                      海康设备，请先下放门禁权限，否则会导致门卡下发失败
                    </span>
                  }
                >
                  <Button
  ...
  const OwnerForm = useCallback((props: any) => {
    const { visible, data, handleSubmit, oncancel } = props
    ...
    console.log(visible,'visiblevisible')
    return (
      <Modal
        visible={visible}
      ...
      render: (text: any, record: any) => (
        <Popover content={getHouseList(record)} trigger="hover">
          <div>
            ...
            {record.householdList &&
              record.householdList.length &&
              record.householdList[0].role === 0 &&
              record.householdList[0].type !== 1 && <Tag color="#f50">家属</Tag>}
            {record.householdList &&
              record.householdList.length &&
            ...
            {record.householdList &&
              record.householdList.length &&
              record.householdList[0].role === 0 &&
              record.householdList[0].type === 1 && <Tag color="#f50">物业</Tag>}
            <label>
              {record.householdList &&
                record.householdList.length &&
